import { HomeIcon, Layers, ListCheckIcon, LogIn, LogOut, User2, UserStar } from "lucide-react";
import { setSignInModalStatus, useTODOStore } from "../Utils/useTODOStore";
  ...
  return (
    <nav className="sticky top-0 z-20 border-b border-slate-800 bg-slate-900/50 p-4 backdrop-blur-md">
      <div className="flex items-center justify-between">
        <h1 className="flex flex-1 items-center gap-2 text-2xl font-bold text-cyan-400">
          <Layers />
          TODO<span className="text-slate-100">Vault</span>
        </h1>

        <div className="flex flex-1 items-center justify-center gap-10">

          <NavLink to="/" className={({ isActive }) => isActive ? "flex items-center gap-1 font-medium text-cyan-400 " : "flex items-center gap-1 text-slate-400 transition hover:text-cyan-300"}>
            <HomeIcon /> Home
          </NavLink>

          <NavLink to="/todo" className={({ isActive }) => isActive ? "flex items-center gap-1 font-medium text-cyan-400 " : "flex items-center gap-1 text-slate-400 transition hover:text-cyan-300"}>
            <ListCheckIcon /> To-Do List
          </NavLink>

        </div>

        {/* <div className="flex items-center justify-end">
          <button onClick={() => setSignInModalStatus(true)} className="flex items-center gap-2 text-slate-400 transition hover:text-cyan-300 cursor-pointer text-md font-medium">
            <LogInIcon /> Sign In
          </button>
        </div> */}
